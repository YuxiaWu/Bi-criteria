...
class PretrainModelManagerParserOri:
    
    def __init__(self, args, data, model, logger_name = 'Discovery'):

        self.logger = logging.getLogger(logger_name)

        args.num_labels = data.n_known_cls_parser
        ...
        self.optimizer = model.set_optimizer(self.model, len(data.dataloader.train_labeled_loader_ori.dataset), args.train_batch_size, \
            args.num_pretrain_epochs, args.lr_pre, args.warmup_proportion)

        self.device = model.device

        self.train_dataloader = data.dataloader.train_labeled_loader_ori  # the parser_ori data
        self.eval_dataloader = data.dataloader.eval_loader_ori  # the parser_ori data
    ...
    def train(self, args, data):
        ...
        for epoch in trange(int(args.num_pretrain_epochs), desc="Pretraining Epoch"):
            ...
            for step, batch in enumerate(tqdm(self.train_dataloader, desc="Pretraining Iteration")):
                ...
                with torch.set_grad_enabled(True):
                    ...
                    logits = self.model(input_ids, segment_ids, input_mask, mode = "pre-train", loss_fct = self.loss_fct, bin_label_ids = bin_label_ids)

                    loss = self.loss_fct(logits, labels_slu)
        ...
        if args.save_model:
            pretrained_model_dir = os.path.join(args.method_output_dir, 'pretrain_parser')
